...
En esta sección se describen los escenarios de prueba ejecutados sobre el sistema para validar el funcionamiento de los módulos principales tanto en el \textit{backend} como en el \textit{frontend}, el cumplimiento de las reglas del negocio y el manejo de respuestas ante distintas interacciones:

\begin{longtable}{|p{1.0cm}|p{2.2cm}|p{3.0cm}|p{3.5cm}|p{3.8cm}|}
	\caption{Matriz detallada de ejecución de casos de prueba del sistema (Backend y Frontend)}
	\label{tab:casos_prueba} \\
	
	\hline
	\textbf{ID} & \textbf{Módulo} & \textbf{Caso de Prueba} & \textbf{Acción / Entrada} & \textbf{Resultado Esperado} \\ \hline
	\endfirsthead
	...
	\textbf{CP-01} & Autenticación & Inicio de sesión exitoso de un Administrador. & Ingresar credenciales válidas en la ruta de acceso y enviar el formulario. & El \textit{backend} valida el hash BCrypt, responde con HTTP 200 y emite un token JWT. \\ \hline
	
	\textbf{CP-02} & Seguridad & Acceso denegado a ruta protegida sin credenciales. & Intentar consumir un \textit{endpoint} protegido del API sin adjuntar el encabezado \texttt{Authorization}. & El filtro JWT intercepta la petición y el servidor responde con un error HTTP 401 Unauthorized. \\ \hline
	
	\textbf{CP-03} & Catálogo & Búsqueda y extracción de portadas mediante API Externa. & Ingresar un título de obra en el buscador para consultar el catálogo. & El \textit{backend} no guarda las portadas, las extrae directamente de Open Library y las integra en el catálogo del libro, retornando HTTP 200 OK. \\ \hline
	
	\textbf{CP-04} & Catálogo & Registro de una nueva obra en el sistema. & Enviar formulario desde el panel para registrar una nueva obra literaria. & El \textit{backend} procesa y persiste la entidad correctamente, retornando HTTP 201 Created. \\ \hline
	
	\textbf{CP-05} & Seguimiento & Validación de límite de progreso (Regla RN-03). & Intentar actualizar la lectura de un libro ingresando un valor mayor al total de páginas de la obra. & El servidor rechaza la transacción y responde con HTTP 400 Bad Request. \\ \hline
	
	\textbf{CP-06} & Seguimiento & Cambio automático de estado a Completado (Regla RN-04). & Registrar el avance final (100\%) en una obra que se encontraba bajo el estado \textit{Leyendo}. & El \textit{backend} actualiza de forma automática el estado de la biblioteca a \textit{Completado} y fija la fecha de término. \\ \hline
	
	\textbf{CP-07} & Gamificación & Actualización atómica de racha de lectura (\textit{streak}). & Registrar una sesión de lectura en el día actual tras haber leído el día anterior. & El caso de uso evalúa la fecha, incrementa la racha consecutiva en $+1$ y persiste el cambio en la BD. \\ \hline
	
	\textbf{CP-08} & Social (Buzón) & Envío de recomendación anónima de formato corto. & Seleccionar una obra leída, redactar una micro-reseña y enviarla al buzón comunitario. & El servidor valida los ciclos de 24 horas, disocia la identidad del remitente y encola la carta de forma anónima. \\ \hline
	
	\textbf{CP-09} & Métricas & Carga de estadísticas globales en el panel analítico. & Navegar a la ruta \path{/estadisticas} para consultar los gráficos de rendimiento e historial. & El controlador calcula y devuelve las métricas agregadas en formato JSON. \\ \hline
	
	\textbf{CP-10} & Frontend (Auth) & Validación de campos vacíos en el formulario de inicio de sesión. & Intentar enviar el formulario de \textit{login} en \path{/login} sin ingresar correo ni contraseña. & El componente de interfaz despliega mensajes de error de validación en tiempo real e impide el envío de la petición HTTP. \\ \hline
	
	\textbf{CP-11} & Frontend (Rutas) & Redirección automática por ausencia de sesión activa. & Intentar acceder directamente a rutas protegidas como \path{/biblioteca} o \path{/estadisticas} sin autenticación previa. & El contexto de autenticación y el enrutador detectan la falta de token y redirigen al usuario hacia la vista de inicio de sesión. \\ \hline
	
	\textbf{CP-12} & Frontend (Biblioteca) & Renderizado dinámico de tarjetas de libros (\textit{BookCard}). & Navegar a la sección de biblioteca y aplicar filtros o paginación sobre los elementos disponibles. & La interfaz actualiza el listado visual de obras de forma responsiva utilizando los componentes de UI y datos del API. \\ \hline
	
	\textbf{CP-13} & Frontend (PWA) & Registro del \textit{Service Worker} y soporte offline básico. & Cargar la aplicación web progresiva y verificar el estado del registro de los recursos estáticos. & El cliente registra correctamente el \textit{Service Worker} permitiendo la caché de activos para funcionamiento offline. \\ \hline
\end{longtable}
...
\addcontentsline{toc}{subsection}{Anexo C: Diccionario de Datos del Sistema Relacional} % <-- ESTO TAMBIÉN
El presente anexo contiene las tablas detallada de la base de datos de \textbf{LecturaMétrica}, elaborados a base de análisis y diseño arquitectónico de la plataforma. 
...
% ==============================================================================
% APÉNDICE: HISTORIAS DE USUARIO
% ==============================================================================
\newpage
\section{Anexo D: Historias de Usuario}
En esta sección se presentan los diagramas de épicas e historias de usuario desarrollados para modelar los requerimientos funcionales y el flujo del sistema, estructurados mediante especificaciones visuales orientadas a objetos.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{./fotos/uc/h/sesion.png} 
	\caption{Historias de usuario correspondientes al progreso de lectura.}
	\label{fig:us_sesion}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{./fotos/uc/h/biblio.png} 
	\caption{Historias de usuario asociadas a la Gestión de Biblioteca y Catálogo de Obras.}
	\label{fig:us_biblio}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{./fotos/uc/h/home.png} 
	\caption{Historias de usuario de la vista principal (\textit{Dashboard} y Resumen del Lector).}
	\label{fig:us_home}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{./fotos/uc/h/buzon.png} 
	\caption{Historias de usuario orientadas al módulo social (Buzón de Recomendaciones Anónimas).}
	\label{fig:us_buzon}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{./fotos/uc/h/user-security.png} 
	\caption{Historias de usuario enfocadas en la Seguridad, Permisos y Configuración de Perfil.}
	\label{fig:us_security}
\end{figure}

\includepdf[pages=-]{./anexos/lm-20Jul.pdf}
